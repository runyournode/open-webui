...
Iterative scans repair the recall, not the shape that causes it: one table, one index, every collection in it. About 70 % of the rows belong to per-file, per-memory and per-web-search collections that are looked up by name and sorted over a handful of rows — they never use the vector index but sit in it. At 3M rows the HNSW index is 10.9 GB where the knowledge bases alone need 6.1 GB, the IVFFlat index is 22.9 GB against a 16.2 GB heap, every search walks a graph that is mostly other collections, and a vacuum that has to clean the index after a knowledge base is deleted takes 59.5 s. Giving each knowledge base its own partition and index — measured on the same 1M dataset, default `ef_search` — gives recall@10 **0.977** at **3.06 ms**, against 0.269 at 4.67 ms as shipped and 0.914 at 8.72 ms with iterative scans; the vacuum drops to 0.8 s and the bucketed 70 % keep recall 1.000. That is the structural proposal in the Discussion.
